  ...
  return (
    <div className="space-y-4">
      ...
      <RadioGroup value={value} onValueChange={onChange} className="space-y-3">
        <div
          onClick={() => onChange('secured')}
          className={cn(
            "flex items-start space-x-3 rounded-lg border-2 p-4 cursor-pointer transition-all hover:border-primary/50",
            value === 'secured' ? 'border-primary bg-primary/5' : 'border-border'
          )}
        >
        ...
        <div
          onClick={() => onChange('unsecured')}
          className={cn(
            "flex items-start space-x-3 rounded-lg border-2 p-4 cursor-pointer transition-all hover:border-primary/50",
            value === 'unsecured' ? 'border-primary bg-primary/5' : 'border-border'
          )}
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Unsecured Loan • Based on co-applicant income (10-12%)
            </p>
          </div>
        </div>
      </RadioGroup>
