...
const SuccessStep = ({
  ...
  const handleLenderSelection = async (lenderId: string) => {
    ...
    try {
      const {
        error
      } = await supabase.from('leads_new').update({
        lender_id: lenderId
      }).eq('case_id', caseId);
  ...
  return <div className="space-y-8 pb-8">
      ...
      {recommendedLenders.length > 0 && <LenderComparisonGrid lenders={sortedLenders} selectedLenderId={selectedLenderId} onSelect={handleLenderSelection} isUpdating={isUpdating} />}

      {/* Selection Confirmation */}
      {selectedLenderId && <div className="p-3 bg-success/10 border border-success/20 rounded-lg text-center">
      ...
      {/* No Lenders Fallback */}
      {recommendedLenders.length === 0 && <div className="rounded-xl border bg-card p-8 text-center">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          num: 1,
          title: 'Lender Review',
          desc: 'Review within 24 hours'
        }, {
          num: 2,
          title: 'Upload Documents',
          desc: 'Complete your application'
        }, {
          num: 3,
          title: 'Get Approved',
          desc: 'Fund your education!'
        }].map(({
          num,
          title,
          desc
      ...
      <div className="text-center space-y-3">
        <div className="flex gap-3 justify-center">
          ...
          <Button variant="outline" size="sm" className="gap-2" onClick={() => toast.info('PDF download coming soon!')}>
