    ...
    <Card className="border-primary/20">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-4 rounded-lg bg-primary/5 border border-primary/20">
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-2xl font-bold text-primary">{stats.newLeadsCount}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Documents</p>
              <p className="text-2xl font-bold text-primary">{stats.documentsAwaitingCount}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Pending Value</p>
              <p className="text-2xl font-bold text-primary">
                ₹{(stats.totalPendingValue / 10000000).toFixed(2)}Cr
              </p>
            </div>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
          ...
          <TabsContent value="all" className="space-y-3 max-h-[400px] overflow-y-auto mt-4">
            {/* New Leads Section */}
            {newLeads.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-muted-foreground flex items-center gap-2">
                  <User className="h-4 w-4" />
                  New Leads Requiring Review
                </h4>
          ...
          <TabsContent value="leads" className="space-y-3 max-h-[400px] overflow-y-auto mt-4">
            {newLeads.length > 0 ? (
              newLeads.map((lead) => (
                <div
                  key={lead.id}
          ...
          <TabsContent value="documents" className="space-y-3 max-h-[400px] overflow-y-auto mt-4">
            {documentsAwaiting.length > 0 ? (
