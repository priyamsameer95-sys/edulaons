...
import { ArrowLeft, User, FileText, CheckCircle, Download, Loader2 } from 'lucide-react';
import { useDocumentTypes } from '@/hooks/useDocumentTypes';
import { useLeadDocuments } from '@/hooks/useLeadDocuments';
import { useLeadInfo } from '@/hooks/useLeadInfo';
import { useBulkDocumentDownload } from '@/hooks/useBulkDocumentDownload';
...
export default function PartnerDocumentPage() {
  ...
  const { isDownloading, downloadAllDocuments } = useBulkDocumentDownload();
  ...
  const handleDownloadZip = () => {
    if (leadId && lead?.case_id) {
      downloadAllDocuments(lead.case_id, leadId);
    }
  };
