-- ===================================================
-- CRITICAL DATABASE REFACTOR: Fix Schema Integrity
-- ===================================================

-- STEP 1: Fix RLS Infinite Recursion (CRITICAL)
-- Drop problematic recursive policies on students table
DROP POLICY IF EXISTS "Students can view their own record" ON students;
DROP POLICY IF EXISTS "Partners can view their own students" ON students;
DROP POLICY IF EXISTS "Partners can update their own students" ON students;
DROP POLICY IF EXISTS "Admins can view all students" ON students;
DROP POLICY IF EXISTS "Admins can update students" ON students;
DROP POLICY IF EXISTS "Active partners can insert students" ON students;
DROP POLICY IF EXISTS "Admins and super_admins can insert students" ON students;

-- Create non-recursive policies using security definer functions
CREATE POLICY "Students: View own record via email" ON students
  FOR SELECT
  TO authenticated
...
CREATE POLICY "Partners: View their students via partner_id" ON students
  FOR SELECT
  TO authenticated
  USING (
    id IN (
      SELECT ln.student_id 
      FROM leads_new ln
      WHERE ln.partner_id = get_user_partner(auth.uid())
    )
  );
...
CREATE POLICY "Partners: Update their students" ON students
  FOR UPDATE
  TO authenticated
  USING (
    id IN (
...
CREATE POLICY "Partners: Insert students" ON students
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
...
-- STEP 2: Add Foreign Key Constraints (Core Relationships)
...
-- STEP 4: Add Missing Indexes for FK Lookups
...
-- STEP 6: Fix Name Validation Trigger
...
-- STEP 7: Add Unique Constraints Where Missing
...
-- STEP 8: Create Missing Composite Indexes for Common Queries
...
DO $$
BEGIN
  RAISE NOTICE 'Schema refactor complete. Foreign keys: %, Indexes optimized', 
