...
BEGIN
  ...
  -- Check orphaned partner references (partner_id can be NULL)
  SELECT COUNT(*) INTO orphaned_count
  FROM public.leads_new l
...
-- These enable Supabase's nested query syntax: students (*), partners (*), etc.

-- Add FK for student_id -> students.id
...
-- Add FK for co_applicant_id -> co_applicants.id
ALTER TABLE public.leads_new
...
-- Add FK for partner_id -> partners.id (nullable)
ALTER TABLE public.leads_new
...
-- Add FK for lender_id -> lenders.id
ALTER TABLE public.leads_new
